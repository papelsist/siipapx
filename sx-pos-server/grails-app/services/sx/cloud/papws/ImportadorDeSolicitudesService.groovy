 ...
 */
@Slf4j
class ImportadorDeSolicitudesService implements ApplicationListener<ContextRefreshedEvent> {
  ...
  private registerListener() {
              ...
              case ADDED:
                try{
                  ...
                  Map errorData = [
                    ...
                    comentario: "Error importando solicitud ${folio} de ${sucursal}",
                    message: message,
                    exception: ExceptionUtils.getRootCause(ex).class.name
                  ]
                  papelsaCloudService.logError(errorData)
    ...
    log.info('Firebase listener para Solicitudes AUTORIZADAS en {}', sucursal)
  }
  ...
  void registrarSolicitud(Map<String, Object> data, String documentId){
    ...
    // sol.fecha = Date.parse("yyyy-MM-dd'T'HH:mm:ss", data.fecha)
    // sol.fechaDeposito =  Date.parse("yyyy-MM-dd'T'HH:mm:ss", data.fechaDeposito)
  ...
  SolicitudDeDeposito createSolicitud(Map data) {
    SolicitudDeDeposito sol = new SolicitudDeDeposito(
      [
        sucursal: sucursal,
        cliente: cliente,
